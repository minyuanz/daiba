...
<div>
  ...
  <div class="BackDetail" v-if="showOrderDetailsPage">
    <h1>訂單詳情</h1>
    <div><p>訂單編號:</p>{{ selectedOrder.ord_id }}</div>
    <div><p>會員編號:</p>{{ selectedOrder.mem_id }}</div>
    <div><p>下單日期:</p>{{ selectedOrder.ord_date }}</div>
    <div><p>訂單地址:</p>{{ selectedOrder.ord_address }}</div>
    <div><p>信用卡號:</p>{{ selectedOrder.ord_credit }}</div>
    <div><p>收件電話:</p>{{ selectedOrder.ord_phone }}</div>
    <div><p>收件姓名:</p>{{ selectedOrder.ord_name }}</div>
    <div><p>都件地址:</p>{{ selectedOrder.ord_address }}</div>
    <div><p>訂單金額:</p>{{ selectedOrder.ord_price }}</div>
    <h1>訂單明細</h1>
    <div class="BackDetailM">
        ...
        <div class="BackDetailD" v-for="detail in selectedOrder.details" :key="detail.prod_id">
...
export default {
  ...
  mounted() {
    axios.get("http://localhost/dai/public/phps/BackOrderM.php").then((response) => {
      this.orders = response.data;
    });
  },
  methods: {
    showOrderDetails(order) {
      axios.get(`http://localhost/dai/public/phps/BackOrderDetailM.php?orderId=${order.ord_id}`)
        .then((response) => {
          this.selectedOrder = {
            ...order, 
